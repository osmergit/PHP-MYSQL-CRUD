...
//...........................
          ...
          $operaciones = $objpersonal->opercerradas2($pagina,$usuario);
          if(sizeof($operaciones) > 0){
               foreach ($operaciones as $row){
                    ...
                    <tr class="w3-hover-light-blue">
                         ...
                         <td class="alt"><?php echo $row['pl'] ?></td>
                         <td><?php echo $row['com_oper'] ?></td>
                          <td><?php echo $row['estado_oper'] ?></td>
                    </tr>
                    <?php
               }
          }
		  ...
		  $objpersonal = new operaciones($pagina);
          $operaciones = $objpersonal->opercerradas3($pagina,$usuario);
          if(sizeof($operaciones) > 0){
               foreach ($operaciones as $row){
                    ?>
                    <tr class="w3-hover-light-blue">
		 ...
		 // sumatoria de las Perdidas o Ganancias 
		   $d =  $row['value_sum'];
         //sumatoria de las perdidas acomuladas


  $objpersonal = new operaciones($pagina);
          $operaciones = $objpersonal->opercerradasresta($pagina,$usuario);
          if(sizeof($operaciones) > 0){
               foreach ($operaciones as $row){
                    ?>
                    <tr class="w3-hover-light-blue">
		  ...
		  //aca termina
		   ...
		   $objpersonal = new operaciones($pagina);
          $operaciones = $objpersonal->opercerradas4($pagina,$usuario);
          if(sizeof($operaciones) > 0){
               foreach ($operaciones as $row){
                    ?>
                    <tr class="w3-hover-light-blue">
					<td>Comisiones por la operación</td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
					...
					<td></td>
					<td></td>
					<td><?php echo $row['value_com'] ?></td>                                              
                      <td></td>  
                      </tr>
                    <?php
               }
          }
          // Suma de las  valor de la comisiones
          $e = $row['value_com'];
          $total = $d;
		  $total1 = ($d/2)+$p;
         // echo $total;
          ...
          if(sizeof($operaciones) > 0){
               foreach ($operaciones as $row){
                    ?>
                    <tr class="w3-hover-light-blue">
					<td>Utilidad Neta ( Rendimiento P/G  - Comision*Opr - Comision Corredor Bolsa </td>
					<td></td>
					<td></td>
					<td></td>
					<td></td>
           ...
           $objpersonal = new operaciones($pagina);
          $operaciones = $objpersonal->opercerradas5($pagina,$usuario);
          if(sizeof($operaciones) > 0){
               foreach ($operaciones as $row){
                    ?>
                    <tr class="w3-hover-light-blue">
					...
					<td></td>
					<td></td>
					<td><?php echo $row['inv_inicial'] ?></td>                                              
                      <td></td>  
                      </tr>
                    <?php
               }
          }
          $inicial = $row['inv_inicial'];
          
		  if ($total > 0)
		  {
		  
          $capital= ($inicial + $total1)  ;
